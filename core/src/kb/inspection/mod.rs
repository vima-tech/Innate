...
/// Observability blocks, KPI snapshots and the metrics delta view.
/// Split out to keep this file under the repo's per-file size limit.
mod metrics;
...
impl KnowledgeBase {
    pub fn inspect(&self) -> Result<Value> {
        ...
        // Endpoint health (last 24h). Read from the LLM/embedding trace log, not
        // from `operation_runs`: a distill whose LLM call failed still records
        // `ok`, because the deterministic fallback succeeded. That is the right
        // behaviour for capture and the wrong signal for health — it is how a
        // stale API key stayed invisible for six days.
        let llm_health = crate::llm_trace::health(24).unwrap_or(Value::Null);
        let llm_degraded = llm_health
            .get("degraded")
        ...
        if llm_degraded {
            suggestions.push(json!({
                "action": "innate inspect | jq .llm_health   # then check settings.json credentials",
                "reason": "LLM/embedding calls are mostly failing — distillation has silently \
                           degraded to the heuristic fallback"
            }));
        }
        if embed_rebuild > 0 {
        ...
        // Every knob that changes ranking or lifecycle, not a subset. The
        // partial list hid `w_lexical`, `w_confidence`,
        // `anti_trigger_penalty` and `repeat_select_conf_max` — and the last
        // of those was silently mis-set relative to the distilled seed
        // confidence, which made a whole archive rule unreachable.
                    let params = json!({
            "recall.w_content": self.w_content,
            "recall.w_trigger": self.w_trigger,
            "recall.w_confidence": self.w_confidence,
            "recall.w_context": self.w_context,
            "recall.w_activation": self.w_activation,
            "recall.w_lexical": self.w_lexical,
            "recall.w_spread": self.w_spread,
            "recall.spread_fan_cap": self.spread_fan_cap,
            "recall.spread_seed_n": self.spread_seed_n,
            "recall.top_k_candidates": self.top_k_candidates,
            "recall.anti_trigger_penalty": self.anti_trigger_penalty,
            "recall.density_refill": self.density_refill,
            "recall.length_penalty": self.length_penalty,
            "recall.length_penalty_free_bytes": self.length_penalty_free_bytes,
            "recall.embed_situation_signature": self.embed_situation_signature,
            ...
            "curate.repeat_select_conf_max": self.repeat_select_conf_max,
            ...
            "curate.failure_min_uses": self.failure_min_uses,
            "curate.failure_max_success_rate": self.failure_max_success_rate,
            "curate.failure_confidence_max": self.failure_confidence_max,
            "evolve.threshold_new_count": self.evolve_threshold,
            "evolve.distill_batch_size": self.distill_batch_size,
            "evolve.schedule_interval_hours": self.evolve_schedule_interval_hours,
            "metrics.retain_days": self.metrics_retain_days,
        ...
        let mut out = json!({
            ...
            "feedback_loop": {
                ...
                // `null`, not 1.0, when nothing ever failed. With zero
                // `task_fail` events on record this metric read a confident
                // 1.0 for months while measuring only that failures were never
                // reported. A missing signal must look missing.
                "task_success_rate": if outcome_known == 0 { Value::Null } else { json!(ratio(succeeded, outcome_known)) },
                "task_outcomes_observed": outcome_known,
                "task_failures_observed": outcome_known - succeeded,
