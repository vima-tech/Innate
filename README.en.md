...
<p align="center">
  <img src="docs/social-preview.png" alt="Innate — 记忆 · 技能 · 直觉 (Memory · Skill · Intuition)" width="860">
</p>
...
## Three Layers — 记忆 · 技能 · 直觉 (Memory · Skill · Intuition)

Innate is built as three cooperating layers over one procedural-knowledge core:

| Layer | What it is | Mechanism |
|---|---|---|
| **记忆 · Memory** | Experience distilled into reusable knowledge that grows from real outcomes | `recall → record → evolve` flywheel; confidence EMA + time decay + curate |
| **技能 · Skill** | Installable, reusable procedural skills, recall-ranked alongside learned knowledge | `kind="skill"` chunks · `innate-memory` SKILL.md · `innate install` wizard |
| **直觉 · Intuition** | A synchronous, no-LLM critic that appraises *footing* before a risky step — returns valence / strength / flagged points, **never an answer** | `innate_appraise` · pure-Rust fused scoring · value-domain-safe |

**Memory** accumulates and sharpens, **Skill** packages and reuses, **Intuition** gut-checks — together they make an agent's procedural know-how compound with use.
...
| Operation | MCP tool |
|---|---|
| Agent may call freely | `innate_recall` · `innate_record` · `innate_appraise` · `innate_evolve` · `innate_inspect` |
...
- Call `innate_appraise(situation=...)` (the **intuition** layer) to gut-check footing before a risky step — it returns what to be careful about, never an answer
...
| `innate appraise` | Intuition | `--query` · `--last-error` · `--recent-actions` · `--stage` · `--file-context` · `--candidate` — returns `{valence, strength, tier, flagged_points}` |
...
### MCP tools (15 tools exposed by `innate mcp`)
...
| `innate_appraise` | **Intuition** critic — appraise footing for a situation; returns `{valence, strength, tier, flagged_points}`, never an answer |
...
│   ├── MCP Server (innate mcp) JSON-RPC 2.0 over stdio, 15 MCP tools
...
- **Three layers in one core**: 记忆 Memory (self-growing knowledge) · 技能 Skill (installable procedures) · 直觉 Intuition (the `innate_appraise` critic) — all over one procedural-knowledge store
- **Intuition critic (`appraise`)**: synchronous, no-LLM footing check that returns `{valence, strength, tier, flagged_points}` and **never an answer** — value-domain-safe against the lethal trifecta
- **Dual-vector recall**: `content_vec` + `trigger_vec`, fused ranking with configurable `w_content / w_trigger / w_confidence / w_activation` weights (the last is an ACT-R recency × frequency signal)
...
- **Native MCP integration**: `innate mcp` exposes 15 tools, works out of the box with Claude Code and Claude Desktop
...
Tests (155 total, all passing) are grouped by responsibility under `core/src/tests/`: `basics` · `feedback` · `governance` · `distillation` · `distill_failures` · `evolve_retries` · `reliability` · `restoration` · `intuition` · `eval` (recall-quality benchmark) · `resilient_distill` (deterministic fallback).
