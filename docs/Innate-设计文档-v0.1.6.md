# Innate 自成长 Agent 程序性知识层设计文档 v0.1.6
...
> 本文描述 Innate 0.1.6 的实际实现。软件发布版本使用 SemVer，数据库 Schema 使用独立迁移版本；两者不要求数值相同。旧版文档保留为历史设计记录，当旧文档与本文或代码冲突时，以当前代码和本文为准。
...
- `manual`：显式执行，不要求已有请求。
- `scheduled`：没有 pending 请求时跳过蒸馏，但仍执行时间衰减、归档等 Curate 维护。
- `threshold`：根据可蒸馏日志数量触发。

Evolve 会领取一条请求并设置 lease；过期的 running 请求可恢复，避免进程异常后永久卡住。
...
Distiller 按日志隔离执行，每条日志可以产出零到多个候选知识。每次调用同时接收同批最多四条相关交互作为重复模式和冲突判断上下文，但输出仍必须归属于当前主日志。一条日志调用或解析失败只把该日志标记为 `failed`，同批其他日志继续处理。

失败日志在 5 分钟冷却后于下一轮 Evolve 的蒸馏阶段前恢复，单日志最多尝试三次；`distill_attempts` 和 `distill_last_failed_at` 持久化保存，因此达到上限后不会形成无限成本循环，成功重试也不会抹除历史失败指标。模型调用设置 30 秒超时和 800 token 输出上限。
...
## 13. 0.1.6 版本与 Schema 演进

Innate 的对外软件版本与数据库 Schema 独立演进。本设计文档对应的软件发布线为 `0.1.6`，当前数据库 Schema 为 `4.13`。
...
- `unknown` outcome 可解析为最终结果；最终结果保持不可变。
...
- related-log 上下文实际传入 Distiller，nomination 经过泛化路径。
- migration chain 最终到达 schema `4.13`。
- 所有对外软件包版本均为 `0.1.6`。
...
3. 增加长期事实压缩策略，在可重放与数据库体积之间取得平衡。

---

**文档结论：** 当前实现使用数据库 Schema 4.13。反馈事实、置信度证据、上下文统计和使用计数已经形成可审计、可重放、支持乱序和修正的闭环；Record 与 LLM 已解耦，蒸馏重试有界且失败历史可观测。剩余重点是语义上下文聚类和更细粒度的因果贡献建模。
